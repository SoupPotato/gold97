...
; Operating in strictly CGB mode, so fill in the
; palette stuff for the status bar.
	ld a, 1
	ldh [rVBK], a
...
.load_pikachu
	depixel 14, 11
	ld a, SPRITE_ANIM_INDEX_UNUSED_PIKACHU
...
; load Pikachu's tail object
	depixel 14, 11
	ld a, SPRITE_ANIM_INDEX_UNUSED_PIKACHU_TAIL
	call InitSpriteAnimStruct
...
PikachuMiniGame_ShowJigglypuff:
	depixel 14, 24
	ld a, SPRITE_ANIM_INDEX_UNUSED_JIGGLYPUFF
	call InitSpriteAnimStruct
...
; this should prevent GBC lagging to shit
	ld a, 24
	ld [wPikachuMinigameFadeOutCounter], a
...
; Skip spawning if y = $FF
	...
	ld a, SPRITE_ANIM_INDEX_UNUSED_NOTE	; NOTE
	call InitSpriteAnimStruct
